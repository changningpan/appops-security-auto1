...
def connectservers(ipaddress, username, password, hostname):
    try:
        ...
        for item in listresult:
            if '/opt/IBM/WebSphere/AppServer' in item:
                list.append(item)
               # if not os.path.exists(ipaddress):
        ...
        if len(list) <= 0:
            ...
            ssh_stdin, ssh_stdout, ssh_stderr = ssh.exec_command('cd /opt/IBM/WebSphere/AppServer/bin ; ./versionInfo.sh')
            ...
            ssh_stdin, ssh_stdout, ssh_stderr = ssh.exec_command('cd /opt/IBM/WebSphere/AppServer/profiles ; ls ')
            for item in ssh_stdout.readlines():
                print(item)
                item1 = item.strip('\n')
               ...
               # pathming=os.getcwd()
                ...
                if len(ssh_stdout.readlines()) > 0:
                    ssh_stdin, ssh_stdout, ssh_stderr = ssh.exec_command('cd /opt/IBM/WebSphere/AppServer/profiles/'+item1+'/config/cells/; ls')
                    for cell in ssh_stdout.readlines():
                        fo1 = open(ipaddress+'/'+item1+'/'+item1 + '.txt', mode='w')
                        #fo2 = open(item1 + 'chmodsecurity.txt', mode='w')

                        commandline = r'head -2 /opt/IBM/WebSphere/AppServer/profiles/' + item1 + '/config/cells/' + cell.strip('\n') + '/security.xml'
                        print(commandline)
                       ...
                       # fo1.write(line.encode('utf-8'))
                        ...
                        commandline2 = 'cd /opt/IBM/WebSphere/AppServer/profiles/%s/config/cells;ls' %(item1.strip('\n'))
                        print(commandline2)
                        ...
                        for num1 in ssh_stdout.readlines():
                            cmd = 'cd /opt/IBM/WebSphere/AppServer/profiles/%s/config/cells/%s;ls -l security.xml'%(item1.strip('\n'),num1.strip('\n'))
                            ...
                            servercmd = 'cd /opt/IBM/WebSphere/AppServer/profiles/%s/config/cells/%s/nodes/;ls'%(item1.strip('\n'),num1.strip('\n'))
                            print(servercmd)
                            ...
                            for node in ssh_stdout.readlines():
                                ...
                                if node1.find('Node') > 0:
                                    #print(node1)
                                    nodecmd = 'cd /opt/IBM/WebSphere/AppServer/profiles/%s/config/cells/%s/nodes/%s/servers;ls' %(item1.strip('\n'),num1.strip('\n'),node.strip('\n'))
                                    print(nodecmd)
                                    ...
                                    for server in ssh_stdout.readlines():
                                        ...
                                        if 'server' in server1:
                                            print('111111server1')
                                            servercmd1 = 'cd /opt/IBM/WebSphere/AppServer/profiles/%s/config/cells/%s/nodes/%s/servers/%s;ls -l server.xml' % (item1.strip('\n'), num1.strip('\n'), node.strip('\n'),server1)
                                            ...
                                            for access in ssh_stdout.readlines():
                                                ...
                                                fo4.write(access)
                                            fo4.close()
                logcmd = 'cd /opt/IBM/WebSphere/AppServer/profiles/' + item1.strip('\n')+'/logs;ls -lR|grep .log$'
                ...
                propertycmd = "cd /opt/IBM/WebSphere/AppServer/profiles/%s/properties;ls -l | egrep 'client.policy|sas.client.props|sas.stdclient.properties|sas.tools.properties|soap.client.props|wsadmin.properties|wsjaas_client.conf|client_types.xml|TraceSettings.properties'" %(item1.strip('\n'))
                ...
                soappropertycmd = "cd /opt/IBM/WebSphere/AppServer/profiles/%s/properties;grep com.ibm.SOAP.login soap.client.props" % (item1.strip('\n'))
                ...
                corbapropertycmd = "cd /opt/IBM/WebSphere/AppServer/profiles/%s/properties;grep com.ibm.CORBA.login sas.client.props" % (item1.strip('\n'))
                ...
                etccmd = "cd /opt/IBM/WebSphere/AppServer/profiles/%s/etc;ls -l *p12" % (item1.strip('\n'))
                ...
                chagecmd = "cd /opt/IBM/WebSphere/AppServer;ls -l"
